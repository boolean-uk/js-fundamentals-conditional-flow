...
let answerFour
if (STR_FIVE[0].toUpperCase === STR_FIVE[STR_FIVE.length - 1].toUpperCase) {
  answerFour = true
} else {
  answerFour = false
}
...
if (STR_SIX[0].toUpperCase() === STR_SIX[STR_SIX.length - 1].toUpperCase()) {
  answerFive = true
} else {
  answerFive = false
}
console.log(STR_SIX[0].toLowerCase())
console.log(STR_SIX[STR_SIX.length - 1].toLowerCase())
// 6. Use conditional statements to set answerSix to the middle character of STR_SEVEN
// if STR_SEVEN has an odd number of characters
const STR_SEVEN = 'Kayla' // eslint-disable-line no-unused-vars

let answerSix
let even = STR_SEVEN % 2 === 0
console.log(even)
if (even) {
  answerSix = null
} else {
