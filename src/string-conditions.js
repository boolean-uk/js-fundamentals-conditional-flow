...
let answerOne
STR_ONE === 'Hello' ? (answerOne = true) : (answerOne = false)

// 2. Use conditional statements to set answerTwo to true if STR_TWO is not 'Hello'
const STR_TWO = 'Goodbye' // eslint-disable-line no-unused-vars

let answerTwo
STR_TWO !== 'Hello' ? (answerTwo = true) : (answerTwo = false)
...
let answerThree
STR_THREE.length > STR_FOUR.length
  ? (answerThree = true)
  : (answerThree = false)
...
let answerSix
STR_SEVEN.length % 2 !== 0 // eslint-disable-line no-unused-expressions
  ? (answerSix = STR_SEVEN[Math.ceil(STR_SEVEN.length / 2) - 1])
  : answerSix
...
let answerSeven
STR_EIGHT.length % 2 === 0 // eslint-disable-line no-unused-expressions
