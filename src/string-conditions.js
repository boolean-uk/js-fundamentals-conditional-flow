...
STR_FIVE.toLowerCase()

if (STR_FIVE.startsWith() === STR_FIVE.endsWith()) {
  answerFour = true
...
if (STR_SEVEN.length % 2 === 1) {
  const middleChar = STR_SEVEN.length / 2

  answerSix = STR_SEVEN.substring(middleChar, middleChar + 1)
}
...
if (STR_EIGHT.length % 2 !== 1) {
  const middleChar = STR_EIGHT.length / 2 - 1

  answerSeven = STR_EIGHT.substring(middleChar, middleChar + 2)
}
