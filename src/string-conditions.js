...
// Check if STR_FIVE starts and ends with the same character, regardless of case
if (
...
if (
  ...
  STR_SIX.charAt(STR_SIX.length - 1).toUpperCase()
) {
  answerFive = true
} else {
  answerFive = false
...
if (STR_SEVEN.length % 2 !== 0) {
  const mid = (STR_SEVEN.length + 1) / 2
  answerSix = STR_SEVEN.charAt(mid - 1)
